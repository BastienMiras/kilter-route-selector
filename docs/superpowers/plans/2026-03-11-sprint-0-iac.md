...
**Architecture:** Two Podman containers (FastAPI backend + Nginx frontend) orchestrated by podman-compose, deployed on a single VPS via Ansible. Three environments live on the same VPS on separate ports (dev :8082, staging :8080, prod :80). Local dev mounts source volumes for hot-reload.
...
```yaml
# Staging environment on VPS — port 8080
